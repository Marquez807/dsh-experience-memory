...
export interface AnchorSuggestion {
  /** In the same spelling `memory_remember`'s `recall_for` accepts. */
  anchor: string
  /** What in the turn makes it a candidate, so a human can accept or reject it quickly. */
  because: string
}

/** The tool calls of the newest turn, oldest first. */
function turnCalls(events: readonly SessionEventLike[]): { name: string; arguments: unknown }[] {
  let start = 0
...
function anchorablePath(path: string): boolean {
  ...
  return path.includes('/') // a directory-bearing path; a bare name fired 508 times once
}
...
export function suggestAnchors(input: {
  /** The turn's events, oldest first. `session.eventsOf(agent)` is the production source. */
  ...
  const seen = new Set<string>()
  const add = (kind: string, token: string, because: string): boolean => {
  ...
  for (const call of turnCalls(input.events).reverse()) {
    ...
    if (facts.command !== '') {
      const token = facts.command.trim().split(/\s+/)[0] ?? ''
