 ...
 */
export function indexRow(record: MemoryRecord): [string, string, string, string, string] {
  const join = (text: string): string => tokenize(text).join(' ')
  return [
    join(record.title),
    join(record.trigger),
 ...
 */
export const BM25_WEIGHTS = [0, 3.0, 4.0, 3.5, 3.5, 1.0] as const
...
export function toRecord(row: Row): MemoryRecord {
  return {
    id: row.id,
 ...
 */
export function upsert(db: DatabaseSync, record: MemoryRecord): void {
  db.prepare(`
    INSERT INTO record (
...
export function getRecord(db: DatabaseSync, id: string): MemoryRecord | undefined {
  const row = db.prepare('SELECT * FROM record WHERE id = ?').get(id) as Row | undefined
...
export function findByFingerprint(
  ...
  const row = scope === 'workspace'
    ...
    ).get(fingerprint, scope, workspaceId) as Row | undefined
    : db.prepare(
      'SELECT * FROM record WHERE content_fingerprint = ? AND scope = ? AND domain = ?',
    ).get(fingerprint, scope, domain) as Row | undefined
  return row === undefined ? undefined : toRecord(row)
}
 ...
 * random uuid strings.
 */
export function candidates(db: DatabaseSync, match: string, limit: number): Candidate[] {
  if (match === '') return []
  return db.prepare(
    'SELECT record_fts.id AS id, bm25(record_fts, ?, ?, ?, ?, ?, ?) AS bm25'
    + ' FROM record_fts WHERE record_fts MATCH ? ORDER BY bm25 LIMIT ?',
  ).all(...BM25_WEIGHTS, match, limit) as Candidate[]
}
...
export function searchRecords(
  ...
  match: string,
  limit: number,
): { record: MemoryRecord; bm25: number }[] {
  if (match === '') return []
  ...
  ).all(...BM25_WEIGHTS, match, limit) as (Row & { bm25: number })[]
...
export function windowRecords(
  ...
  ).all('workspace', workspaceId, 'domain', domain, limit) as Row[]
...
export function workspaceRecordsByFingerprint(db: DatabaseSync, fingerprint: string): MemoryRecord[] {
  const rows = db.prepare(
    'SELECT * FROM record WHERE content_fingerprint = ? AND scope = ?',
  ).all(fingerprint, 'workspace') as Row[]
  return rows.map(toRecord)
}
...
export function noteCorroboration(db: DatabaseSync, fingerprint: string, workspaceId: string, at: number): void {
...
export function confirmedAfter(db: DatabaseSync, after: string, limit: number): MemoryRecord[] {
  const rows = db.prepare(
    'SELECT * FROM record WHERE status = ? AND id > ? ORDER BY id LIMIT ?',
  ).all('confirmed', after, limit) as Row[]
  return rows.map(toRecord)
}

/** Read one `meta` value, or `''`. */
export function readMeta(db: DatabaseSync, key: string): string {
...
/**
 * The most recent audit entry for one record, or `undefined` when it has none.
 *
 * `remember` needs it to tell two retirements apart, because they are not the same
 * decision. A machine retirement (maintenance ageing a record out, a harvest sweep, a
 * fail-streak demotion) is written by `experience-memory` and may be undone by a fresh
 * verified report. An explicit `memory_forget` is written by `agent`, and re-reporting the
 * same content must not quietly reverse a person's "forget this".
 */
export function latestCorrection(
  db: DatabaseSync,
  recordId: string,
): { actor: string; reason: string; at: number } | undefined {
  return db.prepare(
    'SELECT actor, reason, at FROM correction WHERE record_id = ? ORDER BY at DESC, id DESC LIMIT 1',
  ).get(recordId) as { actor: string; reason: string; at: number } | undefined
}
 ...
 */
export function noteRetrieval(db: DatabaseSync, ids: readonly string[], now: number): number {
...
export function failureShapes(db: DatabaseSync, workspaceId: string, limit: number): FailureShape[] {
  const rows = db.prepare(
...
export function failureShapeWorkspaces(db: DatabaseSync, tool: string, shape: string): number {
  const row = db.prepare(
    'SELECT count(*) AS n FROM failure_shape WHERE tool = ? AND shape = ?',
  ).get(tool, shape) as { n: number } | undefined
  return row?.n ?? 0
}

export function countFailureShapes(db: DatabaseSync, workspaceId: string): number {
 ...
 * stops being cheap.
 */
export function evictFailureShapes(db: DatabaseSync, workspaceId: string, limit: number): number {
  const row = db.prepare(
