...
export interface Workspace {
  /** 16 hex characters derived from the resolved root path. */
  id: string
  root: string
  /** Empty means workspace-local only. */
  domain: string
}
...
export function domainFromPackageName(name: string): string {
  return normalizeDomain(name.replace(/^@/, ''))
}

function readConfiguredDomain(root: string): string {
...
function readGitRemoteName(root: string): string {
 ...
 */
export function inferDomain(root: string, configured = ''): string {
...
export function resolveWorkspace(cwd: string | undefined, configuredDomain = ''): Workspace {
