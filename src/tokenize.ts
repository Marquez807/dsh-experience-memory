 ...
 */
export function isIdentifier(span: string): boolean {
 ...
 */
export function identifierKey(phrase: string): string {
  return phrase
    .trim()
    .toLowerCase()
    .replace(/[./:\-\s]+/g, '_')
 ...
 * `index build`.
 */
export function identifiers(text: string, limit = 64): string[] {
  const found: string[] = []
