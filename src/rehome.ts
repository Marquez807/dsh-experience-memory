...
import { getRecord, upsert, writeMeta } from './db.ts'
...
export interface WorkspaceCensus {
  ...
  records: number
  confirmed: number
  /** Records that would be visible to that workspace today (the delivery pool's rule). */
  visible: number
}
 ...
 * rather than guess.
 */
export function censusWorkspaces(db: DatabaseSync, now: number): WorkspaceCensus[] {
  ...
  `).all(now) as { id: string; records: number; visible: number; confirmed: number }[]
  return rows.map(row => ({
    id: String(row.id),
...
export interface RehomePlan {
  from: string
  to: string
  /** Nothing to do: the two identities are the same. */
  same: boolean
  /** Workspace-scoped records that would change hands. */
  records: number
  /** Corroboration rows that would change hands. */
  corroborations: number
  /** Of those, the ones the target already has — same fact, so they are dropped, not moved. */
  mergedCorroborations: number
  /** Records whose content the target already holds; reported, never moved. */
  collisions: number
 ...
 */
export function planRehome(db: DatabaseSync, from: string, to: string): RehomePlan {
  ...
  `).all(from, to) as { id: string; title: string; fingerprint: string }[]
  ...
  for (const row of movable) {
    if (Number((corroborationAt.get(String(row.fingerprint), from) as { n: number }).n) === 0) continue
    if (Number((corroborationAt.get(String(row.fingerprint), to) as { n: number }).n) > 0) mergedCorroborations += 1
...
export interface RehomeResult {
  ...
  records: number
  /** Records left behind because the target already holds that content. */
  skipped: number
  corroborations: number
  mergedCorroborations: number
}
 ...
 */
export function applyRehome(db: DatabaseSync, from: string, to: string, now: number): RehomeResult {
  ...
  ).all(from) as { id: string; fingerprint: string }[]
  ...
  `)
  const moving: { id: string; fingerprint: string }[] = []
  for (const row of rows) {
    const twin = Number((targetWouldCollide.get(to, String(row.fingerprint)) as { n: number }).n)
