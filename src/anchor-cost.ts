 ...
 */
import { ANCHOR_COST_TABLE } from './anchor-cost-table.ts'
import { parseAnchor } from './anchors.ts'

export interface RefusedAnchor {
  /** As the caller wrote it. */
  anchor: string
  /** Measured hits in the corpus the table was built from; `0` for an anchor that cannot parse. */
  hits: number
  /** `hits / calls`, for a message that says how much of every turn this would cost. */
  share: number
  reason: 'too-common' | 'unparseable'
}

export interface AnchorGuardResult {
  /** The anchors to keep, in the order given. */
  kept: string[]
  refused: RefusedAnchor[]
  /** Which snapshot the numbers came from, or `null` when no table is in play. */
  table: { generatedAt: string; calls: number; thresholdHits: number } | null
}

export interface AnchorCostTable {
  generatedAt: string
  corpus: string
  calls: number
...
export function guardAnchors(
  ...
  options: { enabled?: boolean; maxHits?: number; table?: AnchorCostTable } = {},
): AnchorGuardResult {
  const enabled = options.enabled ?? true
  const source = options.table ?? ANCHOR_COST_TABLE
  const counts: Record<string, number> = source.tokens
