...
import { CANDIDATE_LIMIT, visible } from './retrieve.ts'
import { importance } from './rank.ts'
import {
  actingTool,
  anchorsSatisfied,
  baseNameOf,
...
export type RecallReason = 'ok-anchor' | 'no-anchor' | 'no-match'

export type RecallDecision = {
  record: MemoryRecord
  /** The anchors that matched, as written — what a reader needs in order to check the call. */
  matched: string[]
  /** Which anchors the record declared, and where they came from. */
  declared: string[]
  /** `declared` when the record wrote its own anchors, `derived` when they came from `source_ref`. */
  via: 'declared' | 'derived'
  reason: RecallReason
}
...
): { anchors: Anchor[]; via: 'declared' | 'derived' } {
  const declared: Anchor[] = []
...
export function decideForCall(
  ...
  now: number,
  options: { tool?: string; derivedAnchors?: boolean; derivedNeedsMention?: boolean } = {},
): RecallDecision | undefined {
  const useDerived = options.derivedAnchors ?? false
  const useMention = options.derivedNeedsMention ?? true
  const facts: CallFacts = callFacts(options.tool ?? '', argumentsValue)
  ...
  const hits: { record: MemoryRecord; matched: string[]; declared: string[]; via: 'declared' | 'derived'; importance: number }[] = []
  for (const record of pool) {
    ...
    // it only counts under two further conditions, both forced by measurement
    // (docs/DELIVERY-GAPS.md §12.5): the call must be about to *change* that file, and — when
    // `derivedNeedsMention` is on — the record must actually talk about it, i.e. its declared
    // fields name the file rather than merely cite it as where the finding was written down.
    // A declared anchor is the record naming its own trigger and is honoured on any tool.
    if (via === 'derived') {
      if (!actingTool(facts.tool)) continue
      if (useMention && !mentionsFile(record, facts.paths)) continue
    }
  ...
  const specificity = (item: { matched: string[] }): number =>
  ...
  const best = hits[0] as typeof hits[number]
  return {
    record: best.record,
...
/**
 * Whether the record's *declared* fields name any of the files the call touches.
 *
 * The difference between "this lesson is about `tools.js`" and "this lesson was written down in
 * a file that mentions `tools.js`". Only the first should interrupt an edit, and the derived
 * anchor cannot tell them apart on its own — this is what does. Declared fields only, for the
 * same reason matching reads them only: the body is where a record talks about everything else.
 */
function mentionsFile(record: MemoryRecord, paths: readonly string[]): boolean {
  if (paths.length === 0) return false
  const declared = new Set(tokenize(`${record.title}\n${record.trigger}\n${record.failureMode}\n${record.lesson}`))
  for (const path of paths) {
    const base = baseNameOf(path)
    if (base === '') continue
    if (declared.has(base.toLowerCase())) return true
    // A record may name the file without its extension (`tools.js` written as `tools`), which
    // the tokenizer keeps as its own term.
    const stem = base.includes('.') ? base.slice(0, base.lastIndexOf('.')) : base
    if (stem.length >= 4 && declared.has(stem)) return true
  }
  return false
}
...
export function explainCall(
  ...
  facts: { tool: string; paths: string[]; command: string }
  anchored: number
  decision: RecallDecision | undefined
} {
