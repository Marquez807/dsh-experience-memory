...
import { renderRecall } from './inject.ts'
...
export function apply(ctx: Context, config: ExperienceConfig): void {
  ...
  try {
    ...
    const total = db.prepare('SELECT COUNT(*) AS n FROM record').get() as { n: number }
    const confirmed = db.prepare("SELECT COUNT(*) AS n FROM record WHERE status = 'confirmed'").get() as { n: number }
    const anchored = db.prepare("SELECT COUNT(*) AS n FROM record WHERE status = 'confirmed' AND trigger LIKE '%--- anchors ---%'").get() as { n: number }
    ctx.logger?.info(
  ...
  ctx.on('agent/turn-stopping', (payload: { agent?: AgentLike } | undefined) => {
    const now = Date.now()
    try {
      maintain(db, {
        now,
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      standing: {
        type: 'boolean',
        description: 'Mark this as a **standing rule**: it is carried on every turn, whatever the turn is '
          + 'about — for a rule with no trigger word, like "always answer in Chinese", which the query-gated '
          + 'digest can never reach. Use it when a rule holds regardless of topic; do **not** use it for a '
          + 'situation ("this file must be read first"), which belongs in `recall_for` instead. Costs bytes on '
          + 'every turn and is capped by `standingMaxRecords`, so a store where everything is standing is a '
          + 'store where nothing is. Skipping the query gate is all it does: the record still needs a '
          + '`verified-*` grade and `confirmed` status to be shown, so a weak claim marked standing stays silent.',
      },
      failure_mode: { type: 'string', description: 'For an experience: what goes wrong without this lesson.' },      lesson: { type: 'string', description: 'For an experience: the actionable instruction.' },
      scope: {
        type: 'string',
      ...
      schema: {
        ...
        properties: {
          ...
          standing: {
            type: 'boolean',
            required: true,
            description: 'The **effective** standing state: true only when this record will really be '
              + 'carried every turn. A record marked standing whose grade is `inferred` (or which is still '
              + 'a candidate) reports false here, because the resident bar applies to standing records too. '
              + 'When this is false after you asked for it, the fix is a checkable `quote`/`source_ref`, '
              + 'not another re-record.',
          },
          danger_examples: {
            type: 'array',
            items: { type: 'string' },
    ...
    execute: (args: {
      ...
      const result = remember(db, {
        ...
        // Passed through as `undefined` when the caller said nothing — **not** collapsed to `false`.
        // `remember` reads `input.standing ?? existing.standing`, so collapsing would demote a
        // standing rule every time anybody re-reported it, silently: every other field of the record
        // comes back unchanged, and only the always-on behaviour quietly stops. Pinned by
        // `tests/plugin.test.ts` ("saying nothing about standing leaves a standing rule standing").
      ...
      return {
        ...
        standing: result.record.standing && result.record.status === 'confirmed',
