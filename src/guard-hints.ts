...
export interface GuardFacts {
  /** The live store's own path — the one file a destructive script must never touch. */
  realStorePath: string
  /** Where throwaway stores are allowed to live. */
  disposableRoot: string
}

/** A computed counter-example the writer may paste into the record. */
export interface GuardHint {
  /** One line naming both facts, ready to paste. */
  line: string
  /** Which markers in the text made this look like a destructive-action record. */
  because: string[]
  /**
   * True when the text already names a distinctive part of the real store's path or the disposable
   * root. A record that already carries the facts needs no hint — repeating them is noise.
   */
  covered: boolean
}

/** Which markers this text matches, lowercased comparison. Empty means "not a destructive record". */
export function looksDestructive(text: string): string[] {
 ...
 */
export function distinctiveTokens(path: string): string[] {
  return path
    .split(/[\\/]+/)
 ...
 */
export function guardHintFor(input: { text: string; facts: GuardFacts }): GuardHint | undefined {
