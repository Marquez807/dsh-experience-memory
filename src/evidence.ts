...
/**
 * Successful tool calls whose id a caller could cite instead.
 *
 * `route: tool-call` was unreachable in practice for a model-side caller. The grade is
 * matched against a tool result's `callId`, and a model never sees that id as text — a
 * caller reported exactly this: it had a real tool output worth citing and no way to
 * name it, so it wrote prose ("memory_stats（本会话调用输出）") and the record became a
 * candidate that could never be injected. Handing the ids back in the failure reason is
 * the only place the caller is looking, and it turns an unreachable route into one that
 * costs a second call.
 */
function citableCalls(events: readonly SessionEventLike[], limit = 5): { callId: string; name: string }[] {
  ...
  const out: { callId: string; name: string }[] = []
  for (const event of [...events].reverse()) {
    if (event.type !== 'tool/result') continue
    const callId = event.data?.message?.source?.callId ?? event.data?.source?.callId
    if (typeof callId !== 'string' || out.some(entry => entry.callId === callId)) continue
    const blocks = event.data?.message?.content ?? event.data?.content
    const failed = Array.isArray(blocks) && blocks.some(block =>
      typeof block === 'object' && block !== null && (block as { isError?: unknown }).isError === true)
    if (failed) continue
    out.push({ callId, name: names.get(callId) ?? 'tool' })
...
function userMessageContaining(events: readonly SessionEventLike[], quote: string): string | undefined {  for (const event of [...events].reverse()) {
...
export function gradeEvidence(request: EvidenceRequest): EvidenceVerdict {
  ...
  if (sourceRef !== '' && events.length > 0) {
    ...
    // The route is only usable if the caller can name a call, and it cannot see ids.
    const citable = citableCalls(events)
    if (citable.length > 0) {
      tried.push('a successful call from this session can be cited instead — set source_ref to its id: '
        + citable.map(call => `${call.callId} (${call.name})`).join(', '))
    }
  } else if (sourceRef !== '') {
