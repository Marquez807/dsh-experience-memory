...
const EVIDENCE_WEIGHT: Readonly<Record<Evidence, number>> = {
  'verified-tool': 3.0,
  'verified-user': 2.5,
  'verified-file': 2.0,
  inferred: 0.5,
...
export const RESIDENT_EVIDENCE: ReadonlySet<Evidence> = new Set<Evidence>([
...
export interface ImportanceFacts {
  evidence: Evidence
  successCount: number
  reuseCount: number
  failStreak: number
 ...
 */
export function importance(facts: ImportanceFacts): number {
 ...
 */
export function eligibleForResident(record: MemoryRecord, score: number, now: number): boolean {
 ...
 */
export function compareRanked(a: RankedRecord, b: RankedRecord): number {
  if (b.importance !== a.importance) return b.importance - a.importance
  if (a.bm25 !== b.bm25) return a.bm25 - b.bm25
  return a.record.id < b.record.id ? -1 : a.record.id > b.record.id ? 1 : 0
}

/** Human-readable justification, so a recall can explain itself. */
export function explain(record: MemoryRecord, identifierMatches: number, now: number): string {
  const parts = [record.evidence]
