...
import { mapStore, scanForStores, type ExcludedStore, type LegacyStore } from './import.ts'
...
export interface AuditInput {
  /** Directory to search for old `.memory` stores. */
  root: string
  now: number
  /** Injectable so tests can exercise path verdicts without touching a disk. */
  pathExists?: (path: string) => boolean
  /** Injectable so tests can exercise the command check without a real PATH. */
  commandExists?: (name: string) => boolean
}

export interface PathFinding {
  path: string
  /** Record indexes that reference it. */
  records: number[]
  /** Longest existing prefix, absent when not even a drive prefix exists. */
...
export interface AuditResult {
  ...
  rows: AuditRow[]
  pathIssues: {
    /** A path whose final segment genuinely does not exist. */
    missing: PathFinding[]
    /** A path that exists but had prose glued to it by the regex. */
    proseSuffixed: number
  }
  commandIssues: { name: string; records: number[] }[]
  duplicates: {
    /** Groups of record indexes sharing one content fingerprint. */
    exact: number[][]
    near: { i: number; j: number; jaccard: number }[]
  }
  contradictions: {
    polarity: { i: number; j: number; jaccard: number }[]
    subject: { subject: string; records: number[] }[]
  }
  quality: {
    question: number[]
    placeholder: number[]
  ...
  /** The recommendation funnel, step by step. */
  funnel: {
    all: number
    confirmed: number
    proofBacked: number
    notSelfReferential: number
    deduplicated: number
    substantive: number
  }
  recommended: number[]
  injectability: {
    ready: number
    blocked: number
    /** Blocked records that clear the bar on one exact identifier hit. */
...
export function makeCommandExists(pathEnv: string | undefined): (name: string) => boolean {
...
): { kind: 'present' } | { kind: 'missing-segment'; prefix: string | undefined } | { kind: 'prose-suffixed'; prefix: string } {
 ...
 */
export function auditLegacy(input: AuditInput): AuditResult {
  const now = input.now
  ...
  const cmdRefs = new Map<string, number[]>()
  ...
  const near: { i: number; j: number; jaccard: number }[] = []
  ...
  const polarity: { i: number; j: number; jaccard: number }[] = []
  for (let i = 0; i < rows.length; i += 1) {
...
function short(result: AuditResult, index: number): string {
...
function renderAuditReport(result: AuditResult): string {
  const lines: string[] = []
  const p = (text: string): void => { lines.push(text) }
 ...
 * subject label.
 */
function renderCatalogue(result: AuditResult): string {
  ...
  const size = (topics: Map<string, number[]>): number =>
...
function renderTsv(result: AuditResult): string {
  const esc = (value: unknown): string => String(value).replace(/\t/g, ' ').replace(/\r?\n/g, '\\n')
  const missingByRecord = new Map<number, string[]>()
  ...
  const commandByRecord = new Map<number, string[]>()
 ...
 */
function renderSelection(result: AuditResult): string {
...
export function writeAuditReports(result: AuditResult, dir: string): Record<keyof typeof AUDIT_FILES, string> {
