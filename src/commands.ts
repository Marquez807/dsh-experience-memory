...
import { gapReport } from './failure.ts'
...

    {
      ...
      handler: (invocation) => {
        ...
        for (const row of rows.slice(0, limit)) {
          ...
          if (row.lessonNotWorking) {
            lines.push(`      ⚠ 那条记录写完之后，这个错又犯了 ${row.sinceRecord} 次（最近 ${row.shape.recentAt.length} 次里）`)
            lines.push('        可能的原因：记录没在动手那一刻送到、写得太笼统、或者这件事本来就不由记忆管。')
            lines.push('        这不是说记录是错的，只是说它没挡住——值得看一眼再决定改写还是删掉。')
          }
        }
        lines.push('')
        ...
        lines.push('另外有些行根本不是错误：用户打断计划评审、工具被中止也会被记进来。')
        lines.push('每一行都带原始报错，值不值得处理由你判断，报告不替你下结论。')
        return success(lines.join('\n'))
      },
    },
  ]
