...
- **那一行经验提示是每轮无条件付费的**：204 字节，即使这个工作区永远不记任何东西也照付。这是有意的取舍——
  ...
  没有别的副作用）。
- **动手前把经验递到眼前，只看工具参数里的标识符，而且是英文形状的**（路径、文件名、长符号、开关）。
  中文参数值一个都抽不出来，所以纯中文的调用拿不到提示——本工作区 156 条已确认记录里，**44 条完全不含这类标识符，
  无论多相关都送不出去**。每轮常驻注入不受这条限制：它按话题门控，不看参数。判据为什么没改、实测数字、
  以及下一个判据要过的门槛，见 [`docs/DELIVERY-GAPS.md`](docs/DELIVERY-GAPS.md)。
