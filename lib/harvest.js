...
const looksLikeWrapper = (line        )          =>
  WRAPPER_START.test(line) || WRAPPER_RELAY.test(line)

const isQuestion = (sentence        )          => QUESTION.test(sentence)

const isConcrete = (sentence        )          => CONCRETE.some(pattern => pattern.test(sentence))

const hasMarker = (sentence        , markers                   )          => {
...
function firstErrorText(blocks                    )         {
 ...
 */
function failureRecovered(turn                             )                     {
  const toolOf = new Map                ()
  const brokeAt = new Map                                           ()
  let index = -1
 ...
 */
export function harvestFrom(turn                             )                               {
