 ...
 */
import { domainCoreRecords, searchRecords, standingRecords } from './db.js'
                                               
import { compareRanked, eligibleForResident, explain, importance } from './rank.js'
import { identifierKey, identifiers, lexicalQueryTerms, matchExpression, tokenize } from './tokenize.js'
 ...
 */
function sharesTopicalTerm(record              , terms                   )          {
 ...
 */
export function visible(record              , workspaceId        , domain        )          {
...
function statuteStatuses(input               )           {
...
export function identifierMatches(record              , queryIdentifiers                   )         {
 ...
 */
function rankOne(record              , queryIdentifiers                   , now        , bm25 = 0)               {
 ...
 */
export function retrieve(db              , input               )                 {
  const excluded                         = {}
  const drop = (reason        )       => {
 ...
 */
export function retrieveStanding(db              , input               )                 {
  const excluded                         = {}
  const drop = (reason        )       => {
    excluded[reason] = (excluded[reason] ?? 0) + 1
  }
 ...
 */
export function retrieveCore(db              , input           )                 {
  const excluded                         = {}
  const drop = (reason        )       => {
    excluded[reason] = (excluded[reason] ?? 0) + 1
  }
