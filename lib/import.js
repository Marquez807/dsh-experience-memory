...
import { findByFingerprint, upsert } from './db.js'
import { inferDomain, workspaceId } from './domain.js'
import { fingerprint, newId } from './lifecycle.js'
 ...
 */
function classifyStore(memoryDir        )                     {
...
export function scanForStores(root        , maxDepth         = DEFAULT_MAX_DEPTH)             {
  const stores                = []
  const errors           = []
  const excluded                  = []
...
function readStore(memoryDir        , projectRoot        , errors          )                          {
  ...
  if (existsSync(sqlite)) {
    try {
      ...
      try {
        const rows = db.prepare('SELECT record FROM l2').all()                        
 ...
 */
export function mapStore(store             , now        )                {
 ...
 */
export function selectionKey(record              )         {
 ...
 */
export function parseSelection(document         )              {
  ...
  for (const [index, entry] of records.entries()) {
    ...
    const { workspaceId, contentFingerprint } = entry                           
 ...
 */
export function summarizeImport(outcome               , options                    )         {
  const lines           = []
