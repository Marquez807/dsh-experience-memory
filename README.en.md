...
- **Just-in-time delivery requires the record to declare which call it applies to** — `path:<file>`, `tool:<name>`, `command:<token>`, set as `recall_for` when the record is written. **A record that declares nothing never appears just before a call**; it still reaches the per-turn digest and `memory_recall`. That is a trade made after measuring: the previous rule inferred applicability from a shared word, fired a hint on **57% of 15,383 real tool calls**, and a blind sample of 47 of those deliveries found **5** that were about the call — while tightening it enough to remove the noise cut recall to single digits. Why the criterion changed, the numbers, the audit and the failed alternatives are in [`docs/DELIVERY-GAPS.md`](docs/DELIVERY-GAPS.md); `node tools/anchors.mjs` prints the current store's coverage.
...
- **21 个套件** (21 suites) in total, run with one command: `pnpm verify`. What each one covers is in the "tests" section of [`docs/DEVELOPING.md`](docs/DEVELOPING.md).
